        ...
        <div className="flex items-center space-x-4 space-x-reverse">
            ...
            <div className="flex-1">
              ...
              <p className="text-xl font-bold text-cyan-400">
                {(value === undefined || value === null || isNaN(value) || !isFinite(value)) ? '۰' : value.toLocaleString('fa-IR', { maximumFractionDigits: isCurrency ? 0 : 2 })}
              </p>
...
export default function App() {
  ...
  const calculateAll = useCallback(() => {
    ...
    const Q4 = (C4_in > 0 && R > 0) ? P_hr_kg / (C4_in * (R / 100)) : 0;
    const C4 = (Q4 > 0) ? (Q3_in*C3_in + Q9_bleed_in*C9_in) / Q4 : C4_in;
    ...
    const Q5 = Q4 * ROA;
    const C5 = Q5 > 0 ? ((R / 100) * C4 * Q4 + C6_in * Q6_in) / Q5 : 0;
    const Q8 = delta_Cu > 0 ? (P_hr_ton * 1000) / delta_Cu : 0;
    const Q7 = Q8 + Q5 - Q6_in;
    const C8 = C4 - delta_Cu;
    const C7 = Q7 > 0 ? (Q5*C5 - Q6_in*C6_in + Q8*C8) / Q7 : 0;
    const Q13 = (acidDensity > 0) ? (acidConsumption * hourlyOreFeedRate) / (acidDensity * 1000) : 0;
    const Q14 = (acidDensity > 0) ? (Q9_bleed_in * A_con9_bleed) / (1000 * acidDensity) : 0;
    ...
    setResults({
      ...
      Q1: hourlyOreFeedRate, Q2, Q3: Q3_in, Q4, Q5, Q6: Q6_in, Q7, Q8, Q9: Q9_bleed_in, Q10: P_hr_ton, Q11, Q12, Q13, Q14, Q15: 0, Q17: 0, Q18: 0, Q19, Q20: 0,
      C1: 0, C2: 0, C3: C3_in, C4, C5, C6: C6_in, C7, C8, C9: C9_in, C10: 0, C11, C12,
  ...
  const getGeminiSuggestions = async () => {
    ...
    const mockAnalysis = `
### تحلیل کلی طرح
طرح ارائه شده با هزینه تولید تقریبی ۱.۱ میلیارد ریال به ازای هر تن مس، در محدوده اقتصادی و قابل قبولی برای شرایط فعلی بازار قرار دارد. تمرکز اصلی برای بهبود سودآوری باید بر روی کنترل هزینه‌های انرژی و مواد مصرفی باشد.

### نقاط قوت
- **مدل یکپارچه:** محاسبات از سنگ‌شکن تا تولید کاتد به هم متصل هستند که امکان تحلیل دقیق را فراهم می‌کند.
- **مقیاس تولید:** ظرفیت ۵۰ هزار تن در سال، یک مقیاس اقتصادی مناسب برای بهره‌برداری از مزایای مقیاس است.

### نقاط ضعف و ریسک‌های احتمالی
- **وابستگی به قیمت برق:** حدود ۴۵٪ از هزینه‌های عملیاتی مربوط به انرژی است که طرح را به نوسانات قیمت برق حساس می‌کند.
- **مصرف اسید:** هزینه اسید بخش قابل توجهی از هزینه‌های مواد مصرفی است. هرگونه افزایش در قیمت اسید یا مصرف آن به دلیل تغییر کانی‌شناسی، می‌تواند سودآوری را تحت تاثیر قرار دهد.

### پیشنهادات بهینه‌سازی
**بخش فرآیند:**
- **بهینه‌سازی آگلومراسیون:** بررسی دقیق اندازه آگلومرا و میزان اسید مصرفی در این مرحله می‌تواند نفوذپذیری هیپ را بهبود داده و مصرف کلی اسید را کاهش دهد.
- **کنترل دمای الکترولیت:** افزایش دمای الکترولیت در واحد EW (در صورت امکان) می‌تواند ولتاژ سلول را کاهش داده و منجر به صرفه‌جویی قابل توجه در مصرف انرژی شود.

**بخش تجهیزات:**
- **سنگ‌شکن HPGR:** بررسی استفاده از سنگ‌شکن‌های غلتکی فشار بالا (HPGR) به جای سنگ‌شکن‌های مخروطی سنتی. این تجهیزات معمولاً مصرف انرژی ویژه کمتری دارند.
- **آندهای کاتالیستی:** جایگزینی آندهای سربی-کلسیمی-قلعی سنتی با آندهای دارای پوشش کاتالیستی (مانند MMO) می‌تواند ولتاژ سلول را تا ۲۰۰ میلی‌ولت کاهش دهد و مصرف کبالت را حذف کند.

**بخش اقتصادی:**
- **قرارداد بلندمدت برق:** مذاکره برای عقد قرارداد بلندمدت با نرخ ثابت برای تامین برق، ریسک نوسانات قیمت را کاهش می‌دهد.
- **تولید اسید در محل:** برای کارخانه‌های با مقیاس بزرگ، بررسی احداث یک واحد تولید اسید سولفوریک در محل (Acid Plant) می‌تواند هزینه‌ها را به شدت کاهش داده و یک جریان درآمدی جانبی از فروش برق تولیدی ایجاد کند.
    `;

    await new Promise(resolve => setTimeout(resolve, 2000));
    
    setGeminiSuggestions(mockAnalysis);
  ...
  const FormulaTable = () => {
    const formulas = [
        ...
        { desc: 'هزینه تولید', formula: 'Cost/ton = Total OPEX / P', params: 'Total OPEX: کل هزینه عملیاتی سالانه, P: تولید سالانه' },
    ];
